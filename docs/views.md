...
| `views/ThemeSwitcherView.qml` | 460 | Theme picker: a live-filtering search field over a horizontal card carousel (full-height delegate; cross-axis centring lives inside the delegate because a horizontal ListView resets `y`), each card carrying 8 resolved palette dots, a title-cased label and a `position/total` counter, plus the apply status line | `island.themeSwitcher` (`DynamicIsland.qml:605`) | Type to filter, Enter applies the match; Down enters as a user-owned context (hover lock + no pointer-away auto-hide), Tab reaches the strip, Left/Right (or `h`/`l`) walk it, Up returns to the field, Enter applies optimistically; a click selects |
...
| `views/BackgroundPickerView.qml` | 392 | Background picker: a live-filtering search field over a horizontal card carousel with image thumbnails, title-cased label and `position/total` counter, plus apply status line | `island.backgroundPicker` (`DynamicIsland.qml:613`) | Type to filter, Enter applies the background; Left/Right (or `h`/`l`) walk it, Up returns to the field, Down returns to the island; click selects |
